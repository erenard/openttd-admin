...
public class NetworkAdminSender extends NetworkSender {
    ...
    public boolean gameScript(String json) {
        if (json == null) {
            return false;
        } else if (json.length() > Packet.MTU - 3) {
    ...
    /**
     * Register updates to be sent at certain frequencies (as announced in the
     * PROTOCOL packet)
     *
     * @param adminUpdateType Update type
     * @param adminUpdateFrequency Update frequency, setting
     * #ADMIN_FREQUENCY_POLL is always ignored
     */
    ...
    /**
     * Poll the server for certain updates, an invalid poll (e.g. not existent
     * id) gets silently dropped
     *
     * @param adminUpdateType the server should answer for, only if
     * #AdminUpdateFrequency #ADMIN_FREQUENCY_POLL is advertised in the PROTOCOL
     * packet.
     * @param data ID relevant to the packet type, e.g. - the client ID for
     * #ADMIN_UPDATE_CLIENT_INFO. Use UINT32_MAX to show all clients. - the
     * company ID for #ADMIN_UPDATE_COMPANY_INFO. Use UINT32_MAX to show all
     * companies.
     */
     ...
     *
     * @param action Action such as NETWORK_ACTION_CHAT_CLIENT (see
     * #NetworkAction).
